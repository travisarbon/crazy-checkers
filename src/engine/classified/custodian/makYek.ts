/**
 * Mak-yek rule-set convenience factory (Phase 4 Task 29.4).
 */

import type { ClassifiedRuleSet } from '../ClassifiedRuleSet';
import type { ClassifiedGameState } from '../state';
import { createCustodianRuleSet } from './CustodianCaptureEngine';
import { createMakYekConfig } from './makYekConfig';
import type { CustodianMove } from './types';

export function createMakYekRuleSet(): ClassifiedRuleSet<ClassifiedGameState, CustodianMove> {
  return createCustodianRuleSet(createMakYekConfig());
}
