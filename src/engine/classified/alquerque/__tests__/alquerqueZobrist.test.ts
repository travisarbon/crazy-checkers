...
describe('alquerqueZobrist — table determinism', () => {
  ...
  it('side-to-move bit toggles the hash', () => {
    const s = buildState({ config, turn: 'white', pieces: { e5: 'm' } });
  ...
  it('different commanders hash differently', () => {
    const a = buildState({ config, pieces: { e5: 'm' } });
    const b = buildState({ config, pieces: { e5: 'M' } });
    const c = buildState({ config, pieces: { e5: 'b' } });
    const d = buildState({ config, pieces: { e5: 'B' } });
...
describe('alquerqueZobrist — repetition table', () => {
