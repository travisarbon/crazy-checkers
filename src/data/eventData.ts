...
/**
 * P5.1 — Per-event illustrated asset bundle.
 *
 * `icon` is the inline SVG string for the 64×64 ink-on-paper icon;
 * `annotation` is the 80×80 pencil-set overlay.
 * `drawOnDurationMs` controls the per-icon stroke-draw animation
 * (600–900ms per the asset spec).
 */
export interface EventIconAsset {
  readonly icon: string;
  readonly annotation: string;
  readonly drawOnDurationMs: number;
}

/**
 * P5.1 — Lazy loader for an event's illustrated assets. Wired up by
 * `src/data/eventIcons/_index.ts` (auto-generated by the
 * `scripts/generate-event-icons.ts` script).
 */
export type EventIconLoader = () => Promise<EventIconAsset>;
